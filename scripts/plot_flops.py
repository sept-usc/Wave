...
def plot_individual_model(results_df, model_name, model_type):
    ...
    line1 = ax1.plot(results_df['embd'], results_df['measured'], 'o-', 
                   color='blue', label='Actual Bytes', linewidth=2, markersize=6, alpha=0.4)
    line2 = ax1.plot(results_df['embd'], results_df['theoretical'], 's--', 
                   color='green', label='Expected Bytes', linewidth=2, markersize=6, alpha=0.4)
    ...
    line3 = ax2.plot(results_df['embd'], results_df['ratio'], 'o--', 
                   color='red', label='Ratio (Actual / Expected)', 
                   linewidth=2, markersize=6, alpha=0.4)
    ...
    measured_max = max(max(results_df['measured']), max(results_df['theoretical'])) * 1.2
    ...
    filename = f'figs/load_bytes_observation/{model_type}_fp32_global_ld_bytes_diff_n_embd.pdf'
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved {filename}")

def plot_combined_chart(model_data, model_types, colors, model_names, role_label='auto'):
    ...
    # Save combined chart
    filename = f'figs/load_bytes_observation/combined_fp32_global_ld_bytes_diff_n_embd_{role_label}.pdf'
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved {filename}")

def _extract_measured_value(df: pd.DataFrame, file_path: str, role: str | None, model_type: str) -> float:
...
def analyze_memory_misses(
    ...
    for file_path in csv_files:
        try:
            ...
            if batch is None or embd is None:
                raise ValueError(f"Unable to extract batch and embd parameters from filename {filename}")
            ...
            measured_value = _extract_measured_value(df, file_path, role=role, model_type=model_type)
            
            # Calculate theoretical value
            theoretical_value = calculate_theoretical_value(embd, batch, model_type, role=role if role != 'auto' else None)
    ...
    for model_type in model_data:
        if model_data[model_type]:
            model_data[model_type] = pd.DataFrame(model_data[model_type]).sort_values('embd')
    ...
    for model_type in model_types:
        if not model_data[model_type].empty:
            plot_individual_model(model_data[model_type], model_names[model_type], model_type)
    
    # Generate combined chart
    role_label = _normalize_role(role) if role is not None else 'auto'
    batch_label = f"batch{4}"
    plot_combined_chart(model_data, model_types, colors, model_names, role_label=f"{role_label}_{batch_label}")
    ...
    for model_type in model_types:
        if not model_data[model_type].empty:
            ...
            print(model_data[model_type][['embd', 'batch', 'measured', 'theoretical', 'ratio']].to_string(index=False))
    
    print(f"\nPDF charts saved")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze global LD lookup miss bytes vs theoretical estimates.")
    parser.add_argument(
        "--role",
        default="auto",
        choices=["auto", "q", "qkv"],
